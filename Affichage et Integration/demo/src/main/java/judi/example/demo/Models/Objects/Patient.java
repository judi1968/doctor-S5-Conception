...
public class Patient {
	...
	public static Patient[] getAllPatient(Connection connection) throws Exception{
		...
		try {
			...
			patients = new Patient[a];
                resultset =  statement.executeQuery();
				int i = 0;
                while(resultset.next()){
					patients[i] = new Patient();
                    patients[i].setIdPatient(resultset.getInt("id_patient"));
                    patients[i].setNomPatient(resultset.getString("nom"));
                    patients[i].setAge(resultset.getInt("ans"));
					i++;
                }
